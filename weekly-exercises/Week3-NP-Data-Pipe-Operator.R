...
visits_1979 <- np_data %>% filter(Year == 1979) %>% summarize(sum(RecreationVisits))
...
# You will need to use the pipe %>% operator and multiple dplyr functions! 
visits_2020 <- np_data %>% filter(Year == 2020) %>% summarize(sum(RecreationVisits))

# Exercise 4
# Now subtract visits_1979 from visits_2020
visits_difference <- visits_2020 - visits_1979
...
# You will need to use the pipe %>% operator and multiple dplyr functions! 
parks_1979 <- np_data %>% filter(Year == 1979) %>% summarize(n_distinct(ParkName))
...
# You will need to use the pipe %>% operator and multiple dplyr functions! 
parks_2020 <- np_data %>% filter(Year == 2020) %>% summarize(n_distinct(ParkName))

# Exercise 7
# Now subtract parks_1979 from parks_2020
parks_difference <- parks_2020 - parks_1979
